...
// SetCondition sets the node's condition to the desired status and validates that the change is applied.
func SetCondition(cs clientset.Interface, nodeName string, conditionType v1.NodeConditionType, status v1.ConditionStatus) error {
	...
	gomega.Eventually(func() error {
		...
		framework.ExpectEqual(status, gotStatus, fmt.Sprintf("failed: got unexpected %s status on node %s", conditionType, nodeName))
